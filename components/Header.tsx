...
export function Header ()
{
  ...
  return <>
    <div className="flex h-8 items-center justify-center bg-navy px-3 text-center text-[10px] uppercase tracking-[.1em] text-white">Free shipping on orders above ₹4,000 <span className="ml-2 text-gold">• Easy 14-day returns</span></div>
    <header className="sticky top-0 z-30 border-b border-stone bg-cream/95 backdrop-blur">
      <div className="shell relative flex h-[84px] items-center justify-between gap-5">
        <Link href="/" className="flex w-fit items-center gap-2.5 text-navy" aria-label="SCOTLAND home">
          ...
          <span className="font-display text-2xl font-semibold tracking-[.16em] sm:text-3xl">SCOTLAND</span>
        </Link>
        ...
        <div className="flex items-center justify-end gap-3 border-navy/15 text-navy lg:border-l lg:ml-6 lg:pl-6">
          <button className="grid h-10 w-9 place-items-center lg:hidden" onClick={ () => setOpen( !open ) } aria-label="Toggle menu"><span className="text-2xl leading-none">{ open ? '×' : '☰' }</span></button>
